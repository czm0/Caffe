...
template <typename Dtype>
void ActionIRNNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    ClipAdd<Dtype><<<CAFFE_GET_BLOCKS(N_*H_), CAFFE_CUDA_NUM_THREADS>>>(
        N_*H_, t, h_to_gate_.gpu_data(), pre_gate_t);
...
template <typename Dtype>
void ActionIRNNLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    ClipAdd<Dtype><<<CAFFE_GET_BLOCKS(N_*H_), CAFFE_CUDA_NUM_THREADS>>>(
        N_*H_, t, h_to_h_.gpu_data(), dh_t_1);
  }
...
INSTANTIATE_LAYER_GPU_FUNCS(ActionIRNNLayer);
