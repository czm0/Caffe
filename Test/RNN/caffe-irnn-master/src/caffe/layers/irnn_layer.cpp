...
void IRNNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  N_ = this->layer_param_.irnn_param().batch_size(); // batch_size
  H_ = this->layer_param_.irnn_param().num_output(); // number of hidden units
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.irnn_param().weight_filler()));
    ...
    weight_shape.push_back(H_);
    weight_shape.push_back(H_);
    float factor = this->layer_param_.irnn_param().hidden_init_factor();
    ...
    shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(this->layer_param_.irnn_param().bias_filler()));
