...
template <typename Dtype>
void ActionLstmLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  clipping_threshold_ = this->layer_param_.action_lstm_param().clipping_threshold();
  //N_ = this->layer_param_.action_lstm_param().trajectory_len(); // batch_size
  H_ = this->layer_param_.action_lstm_param().num_output(); // number of hidden units
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.action_lstm_param().weight_filler()));
    ...
    shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(
        this->layer_param_.action_lstm_param().bias_filler()));
    bias_filler->Fill(this->blobs_[2].get());

    Dtype* bias_data = this->blobs_[2]->mutable_cpu_data();
    caffe_set(H_, Dtype(2.), bias_data+H_);
  }  // parameter initialization
  this->param_propagate_down_.resize(this->blobs_.size(), true);
}

template <typename Dtype>
void ActionLstmLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  // Figure out the dimensions
  //T_ = bottom[0]->num() / N_; // length of sequence
  T_ = this->layer_param_.action_lstm_param().trajectory_len(); // trajectory_len
  N_ = bottom[0]->height();
  ...
  vector<int> h_to_gate_shape;
  h_to_gate_shape.push_back(N_);
  h_to_gate_shape.push_back(4);
  h_to_gate_shape.push_back(H_);
  h_to_gate_.Reshape(h_to_gate_shape);
}

template <typename Dtype>
void ActionLstmLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    for (int n = 0; n < N_; ++n) {
      ...
      if (cont) {
        caffe_add(4*H_, pre_gate_t, h_to_gate_t, pre_gate_t);
      }
      for (int d = 0; d < H_; ++d) {
        // Apply nonlinearity
...
template <typename Dtype>
void ActionLstmLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(ActionLstmLayer);
#endif

INSTANTIATE_CLASS(ActionLstmLayer);
REGISTER_LAYER_CLASS(ActionLstm);
