...
template <typename Dtype>
__global__ void LSTMClipAdd(const int nthreads, const int dim, int t,
...
template <typename Dtype>
__global__ void LSTMActivationForward(const int nthreads, const int H,
...
template <typename Dtype>
__global__ void LSTMActivationBackward(const int nthreads, const int H,
...
template <typename Dtype>
void ActionLstmLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    LSTMClipAdd<Dtype><<<CAFFE_GET_BLOCKS(4*N_*H_), CAFFE_CUDA_NUM_THREADS>>>(
        4*N_*H_, 4*H_, t, clip_t, h_to_gate_.gpu_data(), pre_gate_t);
    CUDA_POST_KERNEL_CHECK;
    LSTMActivationForward<Dtype><<<CAFFE_GET_BLOCKS(4*N_*H_), CAFFE_CUDA_NUM_THREADS>>>(
...
template <typename Dtype>
void ActionLstmLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    CUDA_POST_KERNEL_CHECK;
    LSTMActivationBackward<Dtype><<<CAFFE_GET_BLOCKS(4*N_*H_), CAFFE_CUDA_NUM_THREADS>>>(
    ...
    LSTMClipAdd<Dtype><<<CAFFE_GET_BLOCKS(N_*H_), CAFFE_CUDA_NUM_THREADS>>>(
...
INSTANTIATE_LAYER_GPU_FUNCS(ActionLstmLayer);
