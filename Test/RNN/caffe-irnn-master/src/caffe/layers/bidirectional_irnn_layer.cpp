...
template <typename Dtype>
void BidirectionalIRNNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  H_ = this->layer_param_.bidirectional_irnn_param().num_output(); // number of hidden units
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.bidirectional_irnn_param().weight_filler()));
    ...
    weight_shape.push_back(H_);
    weight_shape.push_back(H_);
    float factor = this->layer_param_.bidirectional_irnn_param().hidden_init_factor();
    ...
    this->blobs_[4].reset(new Blob<Dtype>(weight_shape));
    hh_weight_data = this->blobs_[4]->mutable_cpu_data();
    caffe_set(this->blobs_[4]->count(), Dtype(0.), hh_weight_data);
    ...
    shared_ptr<Filler<Dtype> > bias_filler(GetFiller<Dtype>(this->layer_param_.bidirectional_irnn_param().bias_filler()));
    bias_filler->Fill(this->blobs_[2].get());

    this->blobs_[5].reset(new Blob<Dtype>(bias_shape));
...
template <typename Dtype>
void BidirectionalIRNNLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  // Figure out the dimensions
  T_ = this->layer_param_.bidirectional_irnn_param().trajectory_len(); // trajectory_len
  ...
  top[1]->Reshape(original_top_shape);
  ...
  backdirect_pre_gate_.Reshape(top_shape);
  top_.Reshape(top_shape);
  backdirect_top_.Reshape(top_shape);
...
template <typename Dtype>
void BidirectionalIRNNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  top_data = backdirect_top_.mutable_cpu_data();
  weight_i = this->blobs_[3]->cpu_data();
  weight_h = this->blobs_[4]->cpu_data();
  bias = this->blobs_[5]->cpu_data();
  pre_gate_data = backdirect_pre_gate_.mutable_cpu_data();
  h_to_gate = h_to_gate_.mutable_cpu_data();
  caffe_set(h_0_.count(), Dtype(0.), h_0_.mutable_cpu_data());
  ...
  for (int t = T_-1; t >=0; --t) {
    ...
    Dtype* pre_gate_t = pre_gate_data + backdirect_pre_gate_.offset(t);
    Dtype* h_to_gate_t=h_to_gate;
    const Dtype* h_t_1 = (t!=(T_-1)) ? (h_t + top_.offset(1)) : h_0_.cpu_data();
  ...
  caffe_copy(N_*H_, top_data, top[1]->mutable_cpu_data());
}

template <typename Dtype>
void BidirectionalIRNNLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  top_data = backdirect_top_.cpu_data();
  top_diff = backdirect_top_.mutable_cpu_diff();
  caffe_set(top_.count(), Dtype(0), top_diff);
  caffe_copy(N_*H_, top[1]->mutable_cpu_diff(), top_diff);

  weight_i = this->blobs_[3]->cpu_data();
  weight_h = this->blobs_[4]->cpu_data();

  pre_gate_data = backdirect_pre_gate_.cpu_data();
  pre_gate_diff = backdirect_pre_gate_.mutable_cpu_diff();

  for (int t = 0; t < T_; ++t) {
    Dtype* dh_t = top_diff + backdirect_top_.offset(t);
    Dtype* pre_gate_diff_t = pre_gate_diff + backdirect_pre_gate_.offset(t);
    Dtype* dh_t_1 = (t!=(T_-1)) ? top_diff + backdirect_top_.offset(t+1) : h_0_.mutable_cpu_diff();
    ...
    // Backprop output errors to the previous time step
    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, N_, H_, H_,
        Dtype(1.), pre_gate_diff + backdirect_pre_gate_.offset(t),
        weight_h, Dtype(0.), h_to_h_.mutable_cpu_data());
    for (int n = 0; n < N_; ++n) {
      const bool cont = (t!=(T_-1));
      const Dtype* h_to_h = h_to_h_.cpu_data() + h_to_h_.offset(n);
  ...
  if (this->param_propagate_down_[3]) {
    // Gradient w.r.t. input-to-hidden weight
    caffe_cpu_gemm(CblasTrans, CblasNoTrans, H_, I_, T_*N_, Dtype(1.),
        pre_gate_diff, bottom_data, Dtype(1.), this->blobs_[3]->mutable_cpu_diff());
  }
  ...
  if (propagate_down[0]) {
    // Gradient w.r.t. bottom data
    caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, T_*N_, I_, H_, Dtype(1.),
        pre_gate_diff, weight_i, Dtype(1.), bottom_diff);
  }

}

#ifdef CPU_ONLY
STUB_GPU(BidirectionalIRNNLayer);
#endif

INSTANTIATE_CLASS(BidirectionalIRNNLayer);
REGISTER_LAYER_CLASS(BidirectionalIRNN);
