...
class ActionIRNNLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ...
  virtual ~ActionIRNNLayerTest() { delete blob_bottom_; delete blob_top_; }
...
TYPED_TEST_CASE(ActionIRNNLayerTest, TestDtypesAndDevices);

TYPED_TEST(ActionIRNNLayerTest, TestGradientDefault) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  if (Caffe::mode() == Caffe::CPU ||
    ...
    ActionIRNNParameter* irnn_param =
        layer_param.mutable_action_irnn_param();
    ...
    ActionIRNNLayer<Dtype> layer(layer_param);
