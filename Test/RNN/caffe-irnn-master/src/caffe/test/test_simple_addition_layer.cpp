...
class SimpleAdditionLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;
 protected:
  SimpleAdditionLayerTest()
  ...
  virtual ~SimpleAdditionLayerTest() { delete blob_bottom_; delete blob_top_; }
...
TYPED_TEST_CASE(SimpleAdditionLayerTest, TestDtypesAndDevices);

TYPED_TEST(SimpleAdditionLayerTest, TestGradientDefault) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  if (Caffe::mode() == Caffe::CPU ||
    ...
    SimpleAdditionParameter* simple_addition_param =
        layer_param.mutable_simple_addition_param();
    simple_addition_param->set_trajectory_len(4);
    this->blob_bottom_vec_.clear();
    this->blob_bottom_vec_.push_back(this->blob_bottom_);

    SimpleAdditionLayer<Dtype> layer(layer_param);
